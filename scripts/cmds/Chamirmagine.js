...
module.exports = {
  config: {
    ...
    cooldowns: 20,
    role: 0,
    shortDescription: "Generate an image using mageV2 API.",
    longDescription: "Generates an image based on the provided prompt using mageV2 API.",
    category: "fun",
    guide: {
      en: "{p}gen <prompt>"
    }
  ...
  onStart: async function ({ message, args, api, event }) {
    ...
    const genApiUrl = `https://www.samirxpikachu.run.place/mageV2?prompt=${encodeURIComponent(prompt)}`;
    ...
    try {
      ...
      fs.writeFileSync(imagePath, genResponse.data);
