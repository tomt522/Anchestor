       ...
       const p = getPrefix(event.threadID);
		...
		if (fs.existsSync(pendingIDsPath)) {
			...
			if (pendingIDs.includes(threadID)) {
				const pendingMsg = "Your request to use bot main cmds is already in pending.... \nPlease contact høpéléss mâhî for fast approval.\n\n Facebook:https://www.facebook.com/profile.php?id=100072881080249\n\n also you can join  anchestor support Box for help\nType : /support \nto join.";
...
for (const adminID of config.adminBot) {
  api.sendMessage(msg, adminID);
		...
		const notifyMsg = `✅Your approval request for using Bot's main cmds has been sent to admin Mahi with this\nmessage: ${userMessage}.\n\n I will notify you if your thread is approved. Please wait until then.\n\n join the anchestor support Box if you facing any problem\nType : /support \nto join.`;
